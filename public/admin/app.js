...
// Novos Elementos (Logins e Pool)
const usersTbody = document.getElementById('users-tbody');
const poolForm = document.getElementById('pool-form');
const poolKeysTextarea = document.getElementById('pool-keys-textarea');
const poolTotalEl = document.getElementById('pool-total');
const poolAvailableEl = document.getElementById('pool-available');
...
async function refreshAllData() {
  try {
    ...
    // Carregar usuários e pool do novo sistema
    loadUsers();
    loadPool();
  } catch (error) {
...
// Configurar o Pool de chaves
function setupPool() {
  if (poolForm) {
    poolForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const keysText = poolKeysTextarea.value;
      
      try {
        const response = await fetch(`${API_URL}/api/admin/pool/add`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ keysText })
        });
        
        if (!response.ok) throw new Error('Falha ao adicionar chaves.');
        
        const data = await response.json();
        showToast(data.message, 'success');
        poolKeysTextarea.value = '';
        refreshAllData();
      } catch (error) {
        showToast(error.message, 'error');
      }
...
// Carregar e renderizar usuários cadastrados
async function loadUsers() {
  try {
    ...
    if (res.ok) {
      ...
      if (users.length === 0) {
        usersTbody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--text-muted);">Nenhum usuário cadastrado até o momento.</td></tr>`;
        return;
      }
      ...
      users.forEach(u => {
        ...
        tr.innerHTML = `
          ...
          <td>${u.email}</td>
          <td><span class="badge badge-active" style="background:rgba(255,255,255,0.05); color:#fff; border:1px solid rgba(255,255,255,0.1);">${u.ip}</span></td>
          <td><span class="key-code">${u.assignedKey}</span></td>
...
// Carregar e renderizar status do pool de chaves
async function loadPool() {
  try {
    const res = await fetch(`${API_URL}/api/admin/pool`);
    if (res.ok) {
      const pool = await res.json();
      
      const total = pool.length;
      const available = pool.filter(k => k.assignedTo === null).length;
      
      poolTotalEl.textContent = total;
      poolAvailableEl.textContent = available;
      
      poolTbody.innerHTML = '';
      
      const distributed = pool.filter(k => k.assignedTo !== null);
      
      if (distributed.length === 0) {
        poolTbody.innerHTML = `<tr><td colspan="3" style="text-align:center; color:var(--text-muted);">Nenhuma chave entregue ainda.</td></tr>`;
        return;
      }
      
      // Ordenar por data de entrega mais recente
      distributed.sort((a, b) => new Date(b.assignedAt) - new Date(a.assignedAt));
      
      distributed.forEach(k => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
          <td><span class="key-code">${k.key}</span></td>
          <td><strong style="color:#fff">${k.assignedTo}</strong></td>
          <td>${formatDate(k.assignedAt)}</td>
        `;
        poolTbody.appendChild(tr);
      });
    }
  } catch (error) {
    console.error('Erro ao buscar pool de chaves:', error);
  }
}
